...
then
  ...
  PRIOR_PNUMBERS=$(grep "Phones:" ${DIR}/install.log | cut -d: -f2)
  ...
  #PHONE_NUMBERS=${PHONE_NUMBERS:-"$PRIOR_PNUMBERS"}
fi
...
# INIT_PHONE - Initial Phone Number to use
# You can specify INIT_PHONE to be equal to the initial phone number to use,
# with other lines being a one off from that number.  You can also specify a single
# phone number to use for all lines or map phone numbers directly to devices.
INIT_PHONE=${INIT_PHONE:-5555550100}
...
# "EXTERNAL_IP_ADDRESS"
# Variable to identify the "Aircraft Network" IP address of the aircraft system.
...
# Extract or Prompt for Phone Numbers
declare -a PNS
if [ "${PHONE_NUMBERS}" ]; then
  PNS=(${PHONE_NUMBERS})
elif [ "${SINGLE_PHONE}" ]; then
  for ((i=1;i<=NMODEMS;i++)); do
    PNS+=("${SINGLE_PHONE}")
  done
else
  # Prompt for the phone numbers
  CPN=${INIT_PHONE}
  for ((i=0;i<$NMODEMS;i++))
  do
    if (( $(echo "${USER_WAIT} <= 0" | bc -l) )); then 
      echo -n "Enter Phone Number for Device ${SDEVS[$i]} [Default: ${CPN}]: "
      read INPUT
    else
      INPUT=""
    fi
...
NPHONES=${#PNS[@]}
if (( NPHONES < NMODEMS )); then die "Not enough phone lines specified"; fi
...
echo "Using the following phone numbers: ${PNS[@]}"
...
echo "Phones:${PNS[@]}"  >> ${TMP_DIR}/install.log
...
then
  ...
  do
    ...
    then
      ...
      $SUDO mv -vf /etc/ppp/chat-${i} /etc/ppp/chat-${i}.${DATE}.bak 2>/dev/null
      $SUDO mv -vf /etc/ppp/peers/${i} /etc/ppp/peers/${i}.${DATE}.bak 2>/dev/null
    fi
  done
fi
...
do
  ...
  # /etc/ppp
  sed "s/ttySX/${DEV}/g;s/.X./.${i}./;s/{PHONENUMBER}/${PHONENUMBER}/g" ${DIR}/etc/ppp/chat-ttySX.baseline > ${TMP_DIR}/chat-${DEV}
  $SUDO install -b -S ".${DATE}.bak" -m 644 -o root -g root -t /etc/ppp ${TMP_DIR}/chat-${DEV}
  # /etc/ppp/peers
  sed "s/ttySX/${DEV}/g;s/.X./.${i}./" ${DIR}/etc/ppp/peers/ttySX.baseline > ${TMP_DIR}/${DEV}
  $SUDO install -b -S ".${DATE}.bak" -m 644 -o root -g dip -t /etc/ppp/peers ${TMP_DIR}/${DEV}

done
$SUDO install -b -S ".${DATE}.bak" -m 755 -o root -g root -t /etc/ppp/ip-up.d/ ${DIR}/etc/ppp/ip-up.d/iptables-up
$SUDO install -b -S ".${DATE}.bak" -m 755 -o root -g root -t /etc/ppp/ip-up.d/ ${DIR}/etc/ppp/ip-up.d/unrealircd
...
# Add ground-proxy to /etc/hosts
$SUDO sed -n -i".${DATE}.bak" -e '/^.*ground-proxy/!p' -e '$a10.155.0.1\tground-proxy' /etc/hosts
...
## STEP 5 - Reconfigure HTTP proxy (SQUID)
sed "s|#{AIRCRAFT_NETWORK}|acl aircraft src ${AIRCRAFT_NETWORK}\nhttp_access allow aircraft|g" ${DIR}/etc/squid3/squid.conf > ${TMP_DIR}/squid.conf
if [ -d /etc/squid3 ]
then
  $SUDO install -b -S ".${DATE}.bak" -m 644 -o root -g root -t /etc/squid3 ${TMP_DIR}/squid.conf
elif [ -d /etc/squid ]
then
  # Location of squid3 configs changed in 16.04 from /etc/squid3 to /etc/squid
  $SUDO install -b -S ".${DATE}.bak" -m 644 -o root -g root -t /etc/squid ${TMP_DIR}/squid.conf
fi
